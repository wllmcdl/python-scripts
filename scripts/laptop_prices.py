...
prices_dec = re.findall(r'class="w-product-price__dec">([0-9]{2})<', str(price))
...
# Concatenate the price + comma + decimal parts through regex
for i in range(len(prices)):
    group = re.search(r'class="w-product-price__main">([0-9]+?)<\/span>(,)<sup class="w-product-price__dec">([0-9]{2})', str(price[i]))
...
# TODO insert the values in a database ( mysql )
